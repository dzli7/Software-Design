...
vec_list_t *make_star(int points, int radius, vector_t center) {
    vec_list_t *star = vec_list_init(2 * points);
    ...
    for (size_t i = 0; i < points; i++) {
        ...
        *inside = vec_add(*inside, center);
        ...
        vec_list_add(star, outside);
        vec_list_add(star, inside);
    }

    return star;
}

vector_t update_position (vector_t velocity, double dt, double radius, vec_list_t *polygon) {
...
int main () {
    ...
    vec_list_t *star = make_star(NUM_POINTS, RADIUS, center);
