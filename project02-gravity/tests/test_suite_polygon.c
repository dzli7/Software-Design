...
// Make square at (+/-1, +/-1)
list_t *make_square() {
    list_t *sq = list_init(4, (free_func_t) vec_list_free);
...
void test_square_area_centroid() {
    vec_list_t *sq = (vec_list_t*) make_square();
...
list_t *make_triangle() {
    list_t *tri = list_init(3, (free_func_t) vec_list_free);
    vector_t *v = malloc(sizeof(*v));
    *v = VEC_ZERO;
    list_add(tri, v);
    v = malloc(sizeof(*v));
...
void test_triangle_area_centroid() {
    vec_list_t *tri = (vec_list_t*) make_triangle();
    assert(isclose(polygon_area((list_t *)tri), 6));
    assert(vec_isclose(polygon_centroid((list_t *)tri), (vector_t) {8.0 / 3.0, 1}));
    vec_list_free(tri);
}
...
void test_triangle_translate() {
    ...
    assert(isclose(polygon_area((list_t *)tri), 6));
    assert(vec_isclose(polygon_centroid((list_t *)tri), (vector_t) {-4.0 / 3.0, -2}));
    vec_list_free(tri);
}
...
void test_triangle_rotate() {
    ...
    assert(isclose(polygon_area((list_t *)tri), 6));
    assert(vec_isclose(polygon_centroid((list_t *)tri), (vector_t) {26.0 / 15.0, 2.2}));
...
list_t *make_big_circ() {
    list_t *c = list_init(CIRC_NPOINTS, (free_func_t) vec_list_free);
...
void test_circ_area_centroid() {
    vec_list_t *c = (vec_list_t*) make_big_circ();
...
list_t *make_weird() {
    list_t *w = list_init(5, (free_func_t) vec_list_free);
    vector_t *v = malloc(sizeof(*v));
    *v = VEC_ZERO;
    list_add(w, v);
    v = malloc(sizeof(*v));
...
void test_weird_area_centroid() {
    vec_list_t *w = (vec_list_t*) make_weird();
    assert(isclose(polygon_area((list_t *)w), 23));
    assert(vec_isclose(polygon_centroid((list_t *)w), (vector_t) {-223.0 / 138.0, -51.0 / 46.0}));
    vec_list_free(w);
}
...
void test_weird_translate() {
    ...
    assert(isclose(polygon_area((list_t *)w), 23));
    assert(vec_isclose(polygon_centroid((list_t *)w), (vector_t) {-1603.0 / 138.0, -971.0 / 46.0}));
...
void test_weird_rotate() {
    ...
    assert(isclose(polygon_area((list_t *)w), 23));
    assert(vec_isclose(polygon_centroid((list_t *)w), (vector_t) {143.0 / 46.0, 53.0 / 138.0}));
